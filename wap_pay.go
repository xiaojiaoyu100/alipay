...
type WapPayParam struct {
	...
	TimeExpire         string                 `json:"time_expire"`                    // 绝对超时时间，格式为yyyy-MM-dd HH:mm。 注：1）以支付宝系统时间为准；2）如果和timeout_express参数同时传入，以time_expire为准。
	TotalAmount        string                 `json:"total_amount"`                   // 订单总金额，单位为元，精确到小数点后两位，取值范围[0.01,100000000]
	AuthToken          string                 `json:"auth_token"`                     // 针对用户授权接口，获取用户相关数据时，用于标识用户授权关系注：若不属于支付宝业务经理提供签约服务的商户，暂不对外提供该功能，该参数使用无效。
	...
	PromoParams        string                 `json:"promo_params"`                   // 优惠参数注：仅与支付宝协商后可用
	...
	QuitUrl            string                 `json:"quit_url"`                       // 添加该参数后在h5支付收银台会出现返回按钮，可用于用户付款中途退出并返回到该参数指定的商户网站地址。注：该参数对支付宝钱包标准收银台下的跳转不生效。
...
func (alipay *Alipay) WapPay(param *WapPayParam, notifyUrl string, returnUrl string) (string, error) {
	...
	if err != nil {
		log.Println("支付宝wap支付构造参数失败: ", err)
		return "", err
	}
